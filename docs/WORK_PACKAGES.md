...
| WP5 | FastAPI backend + chat web UI with citations & source viewer | [WP5-app.md](handovers/WP5-app.md) | ✅ done · 2026-07-26: **PressMeets RAG** — auth (scrypt + signed cookies, **revoked by password change / disable / delete**), token-streaming SSE answers, clickable citations opening the exact passage, **History page** (search inside answers, rename, delete, reopen in chat), **admin console** (roles, password reset, account deletion + history purge) · **79 tests + 31 browser checks with screenshots** · verified against the real API (352 deltas / 17 s, 0 invalid citations) |
| WP6 | Eval harness, hardening, portable export for the target system | [WP6-eval-export.md](handovers/WP6-eval-export.md) | 🚧 in progress · 2026-07-26: **eval harness** (30 grounded questions, free retrieval scoring, opt-in paid answer scoring, baseline deltas) — baseline **hit 0.667 / MRR 0.528**, and k=30 proves the gap is *ranking* not coverage · **portable export** (`EXPORT.json` + checksum + `--verify`, 198 MB → 66.8 MB, real extract-and-query round trip) · **fixed the broken clone-and-run path** (default embed model was the abandoned 768-dim one) · 120 tests · open: reranker, `source_doc` weighting, cross-script facts |
